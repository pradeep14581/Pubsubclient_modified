...
#define MQTT_HEADER_VERSION_LENGTH 7
            ...
            uint8_t v;
            if (willTopic) {
                v = 0x04|(willQos<<3)|(willRetain<<5);//0x04|(willQos<<3)|(willRetain<<5);
            } else {
                v = 0x00;
            }
            if (cleanSession) {
...
//                      }                   
                   Serial.println(" Publish received at QOS2");
                }
                else if(type == MQTTPUBACK)
                {
                    Serial.println(" MQTTPUBACK Message delivered at QOS 1");
                }
                else if(type == MQTTPUBCOMP)
                {
                    Serial.println(" Published delivered to client successfully at QOS2");
                }                
            } else if (!connected()) {
...
boolean PubSubClient::publish_P(const char* topic, const char* payload, boolean retained) {
    return publish_P(topic, (const uint8_t*)payload, strlen(payload),0, retained);
}
